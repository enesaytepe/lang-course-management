...
public sealed class CourseScheduleConfiguration : IEntityTypeConfiguration<CourseSchedule>
{
    public void Configure(EntityTypeBuilder<CourseSchedule> builder)
    {
        ...
        builder.HasQueryFilter(x => !x.Course.Branch.IsDeleted && !x.Course.OfferedLanguage.IsDeleted && !x.Course.CourseLevel.IsDeleted && !x.Course.Teacher.IsDeleted && !x.Course.Classroom.IsDeleted);
    }
}
