(function (window, $) {
    ...
    var page = {
        ...
        canInitialize: function () {
            return ($("#languages-page").length > 0 && $("#languagesTable").length > 0)
        ...
        bindEvents: function () {
            ...
            if (this.$table.length) {
                this.$table
                    .off("click.languagesPage", "[data-language-delete]")
                    .on("click.languagesPage", "[data-language-delete]", function (event) {
            ...
            if (this.$createModal.length && this.$createForm.length) {
                ...
                this.$createModal
                    .off("shown.bs.modal.languagesPage hidden.bs.modal.languagesPage")
                    .on("shown.bs.modal.languagesPage", function () {
                        self.clearCreateForm();
                        self.$createForm.find("[name='Name']").trigger("focus");
                    })
                    .on("hidden.bs.modal.languagesPage", function () {
                        self.clearCreateForm();
                    });
                this.$createForm
                    .off("submit.languagesPage")
                    .on("submit.languagesPage", function (event) {
        ...
        handleStandaloneSubmit: function (event) {
            ...
            var isUpdate = $form.data("language-form") === "update";
            ...
            this.submit($form, isUpdate)
                .done(function (result) {
                    var languageId = result.id || result.Id;
                    window.location.assign("/Language/Details/" + encodeURIComponent(languageId));
                })
                .fail(function (xhr) {
        ...
        submit: function ($form, isUpdate) {
            ...
            return $.ajax({
                url: isUpdate ? "/api/languages/" + encodeURIComponent(id) : "/api/languages",
                ...
                columns.push({
                    ...
                    render: function (data) {
                        ...
                        return '<div class="table-actions">' +
                            '<a class="btn btn-sm btn-outline-primary" href="/Language/Details/' + routeId + '">Detay</a>' +
                            '<a class="btn btn-sm btn-outline-secondary" href="/Language/Edit/' + routeId + '">Düzenle</a>' +
                            '<button type="button" class="btn btn-sm btn-outline-danger" data-language-delete="' + id + '">Sil</button>' +
                            '</div>';
                    }
                });
            }
        ...
        initializeTable: function () {
            this.dataTable = this.$table.DataTable({
                ...
                ajax: function (request, callback) {
                    ...
                    var url = "/api/languages?pageIndex=" + pageIndex
                        + "&pageSize=" + pageSize
        ...
        handleDelete: function (event) {
            var self = this;
            var id = $(event.currentTarget).data("language-delete");
